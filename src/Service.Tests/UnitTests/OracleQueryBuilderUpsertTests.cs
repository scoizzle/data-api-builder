...
namespace Azure.DataApiBuilder.Service.Tests.UnitTests
{
    ...
    public class OracleQueryBuilderUpsertTests
    {
        ...
        public void OracleUpsertContainsInsertBranchWithCreatePolicyGuard()
        {
            ...
            Assert.IsTrue(
                query.Contains("SELECT COUNT(*) INTO", StringComparison.Ordinal),
                $"The create-policy INSERT MUST be gated by SELECT COUNT(*) INTO, not IF (SELECT ...). Query: {query}");
            Assert.IsTrue(
        ...
        /// the policy via SELECT COUNT(*) INTO then IF v_count > 0, and opens an empty cursor
        ...
        public void OracleInsertWithCreatePolicyGatesOnPolicyAndOpensEmptyCursorWhenBlocked()
        {
            ...
            Assert.IsTrue(query.Contains("INSERT INTO", StringComparison.Ordinal), $"Expected an INSERT statement. Query: {query}");
            Assert.IsTrue(
                query.Contains("DECLARE v_dab_insert_count NUMBER", StringComparison.Ordinal),
                $"The INSERT MUST declare a NUMBER for the policy probe (PL/SQL IF cannot host a scalar subquery). Query: {query}");
            Assert.IsTrue(
                query.Contains("SELECT COUNT(*) INTO v_dab_insert_count FROM (SELECT", StringComparison.Ordinal),
                $"The INSERT MUST be gated by SELECT COUNT(*) INTO from a named DUAL subquery so column-referencing policies resolve. Query: {query}");
            Assert.IsTrue(
                query.Contains("IF v_dab_insert_count > 0 THEN", StringComparison.Ordinal),
                $"The INSERT MUST gate on IF v_dab_insert_count > 0, not IF (SELECT ...). Query: {query}");
            Assert.IsTrue(
                query.Contains("WHERE 1 = 0", StringComparison.Ordinal),
        ...
        [TestMethod]
        [TestCategory(TestCategory.ORACLE)]
        public void OracleInsertCreatePolicyProbeUsesDedupedColumns()
        {
            const string createDbPolicy = "(publisher_id != 0)";
            ...
            SqlInsertStructure structure = new(
                ...
                authorizationResolver: authorizationResolver.Object,
                gQLFilterParser: gQLFilterParser,
                mutationParams: new Dictionary<string, object?> { { "id", 1 }, { "title", "New Book" } },
                httpContext: httpContext);

            structure.DbPolicyPredicatesForOperations[EntityActionOperation.Create] = createDbPolicy;
            structure.InsertColumns.Add(structure.InsertColumns[0]);
            structure.Values.Add(structure.Values[0]);

            string query = new OracleQueryBuilder().Build(structure);

            int dualSelectStart = query.IndexOf("FROM (SELECT ", StringComparison.Ordinal);
            int dualSelectEnd = query.IndexOf(" FROM DUAL)", dualSelectStart, StringComparison.Ordinal);
            Assert.IsTrue(dualSelectStart >= 0 && dualSelectEnd > dualSelectStart, query);
            string dualSelect = query.Substring(dualSelectStart, dualSelectEnd - dualSelectStart);
            int idAliasCount = 0;
            int idx = 0;
            while ((idx = dualSelect.IndexOf(" AS \"ID\"", idx, StringComparison.Ordinal)) >= 0)
            {
                idAliasCount++;
                idx += " AS \"ID\"".Length;
            }

            Assert.AreEqual(1, idAliasCount, $"Policy DUAL subquery must not repeat AS \"ID\". Query: {query}");
        }

        /// <summary>
        /// Verifies that fallback-to-update (e.g. autogenerated primary key) produces no INSERT
        /// branch, emits the 'updated' indicator for matched rows, and - when no row matched -
        /// distinguishes a policy-blocked existing row (EMPTY cursor, 403) from a missing row
        /// ('missing' indicator, 404) via a primary-key existence probe.
        /// </summary>
        [TestMethod]
        [TestCategory(TestCategory.ORACLE)]
        public void OracleUpsertFallbackToUpdateHasNoInsertBranch()
        {
            ...
            Assert.IsTrue(
                query.Contains("WHERE 1 = 0", StringComparison.Ordinal),
                $"A policy-blocked existing row MUST open an EMPTY cursor (403). Query: {query}");
            Assert.IsTrue(
                query.Contains($"'{OracleQueryBuilder.MISSING_UPSERT}'", StringComparison.Ordinal),
                $"A missing row MUST emit the '{OracleQueryBuilder.MISSING_UPSERT}' indicator so the executor surfaces 404. Query: {query}");
        }
