...
namespace Azure.DataApiBuilder.Core.Services
{
        ...
        SqlMetadataProvider<OracleConnection, OracleDataAdapter, OracleCommand>
    {
        ...
        /// <summary>
        /// Parses a BEFORE INSERT trigger body to find columns that are populated from
        /// sequences via the pattern: SELECT seq.NEXTVAL INTO :new.column FROM dual
        /// Also handles the IF :new.col IS NULL THEN ... END IF pattern.
        /// </summary>
        private static void ParseTriggerBodyForIdentityColumns(string triggerBody, SourceDefinition sourceDefinition)
        {
            ...
            // Also handle quoted identifiers: :new."ID"
            Regex selectIntoRegex = new(
                @"SELECT\s+(\w+)\.NEXTVAL\s+INTO\s+:new\.(""?\w+""?)\s+FROM\s+dual",
                RegexOptions.IgnoreCase | RegexOptions.Compiled);

            MatchCollection matches = selectIntoRegex.Matches(triggerBody);
            foreach (Match match in matches)
            {
                string columnNameRaw = match.Groups[2].Value;
                // Strip surrounding quotes if present (e.g., "ID" -> ID)
                string columnName = columnNameRaw.Trim('"');

                // Normalize to the casing used in sourceDefinition.Columns keys.
                // Oracle metadata uses uppercase for unquoted identifiers.
                ...
                {
                    // Try lowercase match (some configs use lowercase column names)
                    string lowerColumnName = columnName.ToLowerInvariant();
                    if (sourceDefinition.Columns.TryGetValue(lowerColumnName, out columnDef))
                    {
                        columnDef.IsAutoGenerated = true;
                        columnDef.IsReadOnly = true;
                    }
